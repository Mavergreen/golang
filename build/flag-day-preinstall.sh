...
#          two callers in build/package-pkg.sh and build/package-cross-pkg.sh, and
#          tests/flag-day-preinstall-test.sh.
#
#          The pkg identifier moved from dev.modernmavericks.golang.* to dev.mavergreen.golang.*, and to
...
#          else the old identity left is already handled: the toolchain prefix (/usr/local/goNNN,
#          /usr/local/goNNN-cross) did not move, so this payload overwrites it, and the old updater .app
#          and its agent are retired by shipyard's shared updater/agent-load.in, which the postinstall
#          stage_updater.sh renders already runs.
set -eu
